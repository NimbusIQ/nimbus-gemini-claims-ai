...
const ClaimsIntelligence: React.FC = () => {
  ...
  const presets = [
    ...
    { title: "Texas Code Insight", prompt: "Reference the latest IRC and Texas Department of Insurance requirements for asphalt shingle replacement. List mandatory upgrades that insurance must cover." },
    { title: "Objection Handling", prompt: "Generate a rebuttal for an insurance adjuster who denied 'overhead and profit' on a storm restoration job involving 3 trades." }
  ];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (useThinkingMode && model === 'gemini-2.5-pro') {
        config.thinkingConfig = { thinkingBudget: 1024 }; // Optimized for speed/quality balance
      }

      const response = await ai.models.generateContent({
        model,
        contents: `Act as a Senior Public Adjuster and Insurance Claims Specialist. ${prompt}`,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div>
              ...
              >
                <option value="gemini-2.5-pro">Deep Analysis (Pro + Thinking)</option>
                <option value="gemini-2.5-flash">Rapid Scan (Flash)</option>
              </select>
            </div>
      ...
      {(isLoading || result || error) && (
        <Card>
          <div className="p-6">
            <h3 className="text-lg font-semibold mb-2 text-indigo-400">Intelligence Report</h3>
            {isLoading && (
              <div className="flex items-center justify-center p-8">
                <div className="flex items-center space-x-3">
                  <Spinner />
                  <span>Cross-referencing Xactimate codes and local statutes...</span>
                </div>
              </div>
            )}
